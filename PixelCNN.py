# from: https://github.com/anordertoreclaim/PixelCNN/blob/606ebe9ae1c68d4bf748c502300752b8cb3a73fc/pixelcnn/model.py
...
class PixelCNN(nn.Module):
    def __init__(self, args):
        ...
        self.hidden_fmaps = 30 # cfg.hidden_fmaps
        self.color_levels = 2 # cfg.color_levels
        self.out_hidden_fmaps = 10 # cfg.out_hidden_fmaps
        self.hidden_kernel_size = 7 # cfg.hidden_k_size
        self.causal_kernel_size = 7 # cfg.causal_ksize
        self.hidden_layers = 6 # cfg.hidden_layers
        self.causal_conv = CausalBlock(DATA_CHANNELS,
                                       self.hidden_fmaps,
                                       self.causal_kernel_size,
                                       data_channels=DATA_CHANNELS)

        self.hidden_conv = nn.Sequential(
            *[GatedBlock(self.hidden_fmaps, self.hidden_fmaps, self.hidden_kernel_size, DATA_CHANNELS) for _ in range(self.hidden_layers)]
        )
    ...
    def sample(self, count, shape, channels=3, label=None, device='cuda'):
        height, width = shape
